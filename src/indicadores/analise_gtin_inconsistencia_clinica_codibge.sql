...
-- ============================================================================
-- ANALISE GTIN X INCONSISTENCIA CLINICA - TESTE MUNICIPIO
-- ============================================================================
-- OBJETIVO: Persistir, para um municipio especifico, a evolucao anual por
...
DECLARE @CodIbge    CHAR(7)     = '4205407';
...
BEGIN
    RAISERROR('Tabela temp_CGUSC.fp.dados_farmacia sem colunas obrigatorias id/cnpj/codibge.', 16, 1);
    RETURN;
END;
...
BEGIN
    RAISERROR('Municipio de teste nao encontrado em temp_CGUSC.fp.dados_farmacia.', 16, 1);
    RETURN;
END;
...
BEGIN
    RAISERROR('Municipio de teste possui ids duplicados em temp_CGUSC.fp.dados_farmacia.', 16, 1);
    RETURN;
END;
...
BEGIN
    RAISERROR('Municipio de teste possui CNPJs duplicados em temp_CGUSC.fp.dados_farmacia.', 16, 1);
    RETURN;
END;
...
WHERE F.codibge = @CodIbge;

SET @QtdLinhas = @@ROWCOUNT;
SET @TempoTrechoMs = DATEDIFF(MILLISECOND, @InicioTrecho, SYSDATETIME());
INSERT INTO #perfil_execucao (trecho, linhas, tempo_ms, acumulado_ms)
VALUES ('PASSO 1A.1 - carregar farmacias do municipio', @QtdLinhas, @TempoTrechoMs, DATEDIFF(MILLISECOND, @InicioProcesso, SYSDATETIME()));
RAISERROR('Tempo PASSO 1A.1 - carregar farmacias do municipio: %d ms | linhas: %d', 0, 1, @TempoTrechoMs, @QtdLinhas) WITH NOWAIT;
...
BEGIN
    RAISERROR('Municipio de teste possui farmacia com id/cnpj obrigatorio nulo.', 16, 1);
    RETURN;
END;
SET @TempoTrechoMs = DATEDIFF(MILLISECOND, @InicioTrecho, SYSDATETIME());
INSERT INTO #perfil_execucao (trecho, linhas, tempo_ms, acumulado_ms)
...
SET @TempoMs = DATEDIFF(MILLISECOND, @InicioEtapa, SYSDATETIME());
INSERT INTO #perfil_execucao (trecho, linhas, tempo_ms, acumulado_ms)
VALUES ('PASSO 1A - total farmacias do municipio', @QtdLinhas, @TempoMs, DATEDIFF(MILLISECOND, @InicioProcesso, SYSDATETIME()));
RAISERROR('Tempo PASSO 1A - farmacias do municipio: %d ms | linhas: %d', 0, 1, @TempoMs, @QtdLinhas) WITH NOWAIT;
...
-- ============================================================================
-- PASSO 2: MOVIMENTOS MONITORADOS DO MUNICIPIO
-- ============================================================================
SET @InicioEtapa = SYSDATETIME();
SET @InicioTrecho = SYSDATETIME();
...
BEGIN
    RAISERROR('Nao ha itens monitorados para o municipio de teste no periodo.', 16, 1);
    RETURN;
END;
