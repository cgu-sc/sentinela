...
const regiaoLocalidades = computed(() => {
  ...
  return geoStore.localidades.filter(
    (l) => String(l.id_regiao_saude) === String(id_reg) && l.sg_uf === uf,
  );
});

const regiaoIbge7Set = computed(
  () =>
    new Set(
...
watch(
  ...
  ([uf]) => {
    ...
    const features = fullGeo.features.filter((f) =>
      ibge7s.has(Number(f.properties.id)),
    );
...
const mapData = computed(() => {
  ...
  const normalize = (s) =>
    s
      ...
      .replace(/[\u0300-\u036f]/g, "")
      .trim() || "";
  const targetMun = props.regionalData?.municipios?.find(
    (m) => Number(m.id_ibge7) === Number(effectiveId),
  )?.municipio;
  ...
  return fullGeo.features
    ...
    .map((f) => {
      ...
      const opacity = !!effectiveId && !isSelected ? 0.85 : 1;
...
watch(
  ...
  async ([ibgeId]) => {
    ...
    const match = mapData.value.find(
      (d) => Number(d.ibge7) === Number(effectiveId),
    );
    if (match?.name) {
  ...
  async (municipioName) => {
    ...
    if (_prevHoveredName) {
      chart.dispatchAction({
        type: "downplay",
        seriesIndex: 0,
        name: _prevHoveredName,
      });
      _prevHoveredName = null;
    }

    if (municipioName) {
      const target = municipioName
        .toLowerCase()
        .normalize("NFD")
        .replace(/[\u0300-\u036f]/g, "");
      const match = mapData.value.find(
