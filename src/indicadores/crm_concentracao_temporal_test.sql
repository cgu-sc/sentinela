...
DROP TABLE IF EXISTS temp_CGUSC.fp.crm_concentracao_alertas;
DROP TABLE IF EXISTS temp_CGUSC.fp.crm_concentracao_controle;
GO
...
IF OBJECT_ID('temp_CGUSC.fp.crm_concentracao_controle') IS NULL
    CREATE TABLE temp_CGUSC.fp.crm_concentracao_controle (
        cnpj       VARCHAR(14) NOT NULL,
        dt_inicio  DATETIME    NOT NULL,
        dt_fim     DATETIME    NULL,
        nu_alertas INT         NULL,
        status     VARCHAR(12) NOT NULL,
        CONSTRAINT PK_ConcentracaoControle PRIMARY KEY CLUSTERED (cnpj)
    );

IF OBJECT_ID('temp_CGUSC.fp.crm_concentracao_alertas') IS NULL
BEGIN
    CREATE TABLE temp_CGUSC.fp.crm_concentracao_alertas (
        cnpj                VARCHAR(14) NOT NULL,
    ...
    CREATE CLUSTERED INDEX IDX_ConcentracaoAlertas
        ON temp_CGUSC.fp.crm_concentracao_alertas(cnpj, dt_dia);
END
...
DELETE alerta
FROM temp_CGUSC.fp.crm_concentracao_alertas alerta
INNER JOIN temp_CGUSC.fp.crm_concentracao_controle ctrl
    ON ctrl.cnpj = alerta.cnpj
WHERE ctrl.status = 'PROCESSANDO';

DELETE FROM temp_CGUSC.fp.crm_concentracao_controle
...
WHERE A.crm    IS NOT NULL
  ...
  AND A.cnpj NOT IN (
      SELECT cnpj FROM temp_CGUSC.fp.crm_concentracao_controle WHERE status = 'OK'
  );

SET @nu_pendentes      = (SELECT COUNT(*) FROM #cnpjs_pendentes);
SET @nu_ja_processados = (SELECT COUNT(*) FROM temp_CGUSC.fp.crm_concentracao_controle WHERE status = 'OK');
...
BEGIN
    ...
    INSERT INTO temp_CGUSC.fp.crm_concentracao_controle (cnpj, dt_inicio, status)
    ...
    INSERT INTO temp_CGUSC.fp.crm_concentracao_alertas
    ...
    SET status     = 'OK',
        ...
        nu_alertas = ISNULL((
            SELECT COUNT(*)
            FROM temp_CGUSC.fp.crm_concentracao_alertas a
            WHERE a.cnpj = ctrl.cnpj
        ), 0)
    FROM temp_CGUSC.fp.crm_concentracao_controle ctrl
...
FROM temp_CGUSC.fp.crm_concentracao_alertas
GROUP BY
    severidade,
