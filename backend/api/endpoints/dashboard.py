...
def get_dashboard_resumo(
    ...
    return DashboardService.get_dashboard_data(db, data_inicio, data_fim, perc_min, perc_max, val_min, uf, regiao_saude, municipio, situacao_rf, conexao_ms, porte_empresa, grande_rede, unidade_pf, regiao_id, id_ibge7)
...
def get_resultado_faixas_risco(
    ...
    if municipio and municipio != "Todos":
        raise HTTPException(status_code=400, detail="Use id_ibge7 para filtros municipais; municipio textual e apenas label.")
    return DashboardService.get_fator_risco_data(db, data_inicio, data_fim, perc_min, perc_max, val_min, uf, regiao_saude, municipio, situacao_rf, conexao_ms, porte_empresa, grande_rede, unidade_pf, regiao_id, id_ibge7)
